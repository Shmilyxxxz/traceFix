...
namespace ASM.TraceabilityProxy.Service
{
	...
	[Serializable]
	public class TraceabilityData2_x
	{
		...
		[XmlIgnore]
		private List<Panel2_1> m_Panels = null;
		...
		public string __DateBegin
		{
			get
			{
				return DateTimeHelper.DateToTraceHostOldFormat(this.m_DateBegin);
			}
			set
			{
		...
		public string __DateComplete
		{
			get
			{
				...
				{
					result = DateTimeHelper.DateToTraceHostOldFormat(this.m_DateComplete.Value);
				}
				return result;
			}
			set
		...
		[XmlElement(ElementName = "panel", Type = typeof(Panel2_1))]
		public List<Panel2_1> Panels
		{
			get
			{
		...
		public TraceabilityData2_x()
		{
			this.m_strVersion = "2.x";
			this.m_nErrorLevel = 0;
		}

		public static string Serialize(TraceabilityData2_x td)
		{
			string result;
			try
			{
				XmlSerializer xmlSerializer = new XmlSerializer(typeof(TraceabilityData2_x));
		...
		{
			return TraceabilityData2_x.Serialize(this);
		}

		public static TraceabilityData2_x Deserialize(string xmlData)
		{
			XmlSerializer xmlSerializer = new XmlSerializer(typeof(TraceabilityData2_x));
			TraceabilityData2_x result;
			using (StringReader stringReader = new StringReader(xmlData))
			{
				using (XmlReader xmlReader = new XmlTextReader(stringReader))
				{
					result = (TraceabilityData2_x)xmlSerializer.Deserialize(xmlReader);
				}
			}
			return result;
		}
